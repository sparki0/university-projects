...
public:
    BinaryHeap() = default;
    explicit BinaryHeap(Comp comp) : _comparator(comp)
    {
    }
    ...
    T extract_min() {
        ...
        std::swap(_tree[0], _tree.back());
        T res = std::move(_tree.back().value);
        _tree.pop_back();
        bubbleDown(0);
        return res;
    }

    void bubbleDown(size_t startInd) {
        for(size_t i = startInd; i < _tree.size() || i != _tree.size(); ) {
            size_t left = 2 * i + 1;
            size_t right = left + 1;
            size_t choosenOne;
            if(left >= _tree.size()) {
            ...
            else if(right >= _tree.size()) {
                choosenOne = left;
            }
            else {
                choosenOne = _comparator(_tree[left].value,_tree[right].value) ? left : right;
            }

            if( _comparator(_tree[i].value, _tree[choosenOne ].value) ){
                return ;
            }
            std::swap(_tree[i],_tree[choosenOne]);
            i = choosenOne;
        }
    }
...
//    std::vector<Ref> _tree;
//    Comp _comparator;

    Ref push(T val) {
        _tree.emplace_back(std::move(Node(std::move(val))));
        Ref result(_tree.back());
//        std::cout << result.get() << std::endl;
        bubbleUp(_tree.size() - 1);
        return result;
    }

    void bubbleUp(size_t startInd) {
        for(size_t i = startInd; i != 0 ;) {
            size_t batya = (i - 1) / 2;
            if( _comparator(_tree[batya].value, _tree[i].value) ) {
                return;
            }
            std::swap(_tree[batya], _tree[i]);
            i = batya;
        }
    }
    ...
    template < typename Fun >
    void change(const Ref& ref, Fun f) {
        size_t ind = ref_to_index(ref);
        f(_tree[ind].value);
        if(ind != 0 && _comparator(_tree[ind].value, _tree[TestHelper::parent(ind)].value)) {
            bubbleUp(ind);
        }
        else {
            bubbleDown(ind);
    ...
    T erase(const Ref& ref) {
        size_t ind = ref_to_index(ref);
        if(ind == 0) {
            return std::move(extract_min());
        }
        std::swap(_tree[ind], _tree.back());
        Node res = std::move(_tree.back());
        _tree.pop_back();
        if( _comparator(_tree[ind].value, _tree[TestHelper::parent(ind)].value)) {
            bubbleUp(ind);
        } else {
            bubbleDown(ind);
        }
    ...
    struct TestHelper {
        static const T& index_to_value(const BinaryHeap& H, size_t index) { return H._tree[index].value; }

        static size_t root_index() { return 0; }
        static size_t parent(size_t index) { return (index - 1)/ 2; }
        static size_t child(size_t parent, size_t ith) { return parent * 2 + 1 + ith ; }
    };
...
int main() {
    ...
    try {
        ...
        run_test<Tester<int>>(500'000, CHANGE );
