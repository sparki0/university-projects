...
std::vector<Vertex> getCycle(const Graph& G, std::vector<size_t> & degCounts) {
    auto reverse = G.reversed();
    std::vector<Vertex> res;

    for(size_t i = 0; i < reverse.vertices(); ++i) {
        if(degCounts[i] == 0) {
            continue;
        }
        std::vector<size_t> parent(G.vertices(), size_t(-1));
        ...
        while( ! q.empty() ) {
            ...
            q.pop();
            for(auto & x : reverse[Vertex(current)]) {
                if(x == i) {
                    for(size_t j = current; j != size_t(-1) ; j = parent[j]) {
                        res.emplace_back(Vertex(j));
                    }
                    return res;
                }
                if( !visited[x] ) {
...
std::pair<bool, std::vector<Vertex>> topsort(const Graph& G) {
    std::queue<Vertex> q;
    std::vector<Vertex> res;
    std::vector<size_t> degCounts(G.vertices(), 0);
    for( const auto & i: G ) {
